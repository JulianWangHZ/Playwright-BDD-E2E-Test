...
一套採用 **Playwright** 與 **pytest-bdd** 技術棧構建的企業級端對端測試框架，專為確保網頁應用程式的品質與可靠性而設計。本框架基於現代瀏覽器自動化原則，實作 Page Object Model 架構與行為驅動開發方法論，實現可維護、可擴展且穩健的測試自動化。框架充分運用 Playwright 的智慧型自動等待機制，有效消除不穩定測試，並提供與 Playwright MCP 的無縫整合，加速測試開發流程並支援互動式除錯工作流程。
...
本測試自動化框架旨在確保網頁應用程式的品質與可靠性，涵蓋關鍵使用者流程與業務流程。

### 目標應用程式
- **網站**: [https://staging.inline.app/](https://staging.inline.app/)
...
class HomePageLocators:
    """首頁定位器，使用 Playwright Locator 對象"""
